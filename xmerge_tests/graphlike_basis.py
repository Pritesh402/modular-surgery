...
def graphlike_basis(in_path, out_path, noise=1e-3, max_passes=12, validate=True):
    ...
    for ins in c.flattened():
        if ins.name == "DETECTOR":
            ...
            obs.append(ins)          # keep EVERY observable, not just the last
    if not obs:
    ...
    # ---- validation ----
    # search_for_undetectable_logical_errors here takes 30+ min at d5 and gets
    # OOM-killed at d7+.  xmerge_diagnose.py does this properly as its own
    # pipeline step, so --no-validate skips it without losing any check.
    if not validate:
        print("  (validation skipped: --no-validate; run xmerge_diagnose.py instead)")
        return True
    ok = True
    try:
...
def main():
    ...
    ap.add_argument("--no-validate", action="store_true",
                    help="skip the built-in validation (30+ min at d5, OOMs at "
                         "d7+). xmerge_diagnose.py covers it.")
    ...
    ok = graphlike_basis(args.input, out, noise=args.noise, validate=not args.no_validate)
    print(f"  wrote {out}")
